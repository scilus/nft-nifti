...
public class NiftiStreamUtilTest {
    ...
    @Test
    void testGetDataStreamTruncation() throws IOException, InterruptedException {
        this.expected.clear();
        ...
        new IndexIterator()
                ...
                .stream()
                .forEach(it -> {
                    this.expected.add(0.123456);
                    this.volume.getData().set(it, 0.123456789);
                });

        DataInputStream stream = NiftiStreamUtil.getDataReadStream(this.volume, 6);
        List<Double> result = new ArrayList<>();
